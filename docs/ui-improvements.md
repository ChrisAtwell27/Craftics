...
- **Through-wall highlight pass.** Highlights are depth-tested, so tiles behind obstacles vanish at low camera angles. A second, faint depth-ignoring pass (or `RenderLayer` with `ALWAYS` depth) would keep the move region readable without rotating the camera. *(→ implemented in round 2, §6.1 - with a GREATER depth test rather than ALWAYS, so the ghost shows only where occluded.)*
...
- **Enemy act-order strip.** During the enemy phase, a centered strip of unit portraits below the turn banner (and below the party turn-order panel in multiplayer, via a shared bottom-Y handoff) shows the order units will act. No payload extension was needed: the enemy sync order already mirrors the server's `enemies` list, which *is* the act order - `CombatState` now records it (`getUnitActOrder`). The unit currently acting - learned from the attack-animation event, so best-effort with a 2.5 s freshness window (`noteActingEnemy`/`getActingEnemyId`) - gets a gold frame; a green/red underline under each portrait reads friend/foe at a glance. Caps at 10 portraits plus a "+N" tail. *(→ round 3: the highlight now fires on movement-only turns too - the server announces walks, teleports and ceiling hops via `EVENT_MOVED`, with the camera following the mover; the freshness window widened to 4 s so long walks don't expire mid-move.)*
- **HP text only when damaged.** Enemy roster rows draw the gray `hp/max` numbers only once the enemy has taken damage - a full bar already says "full", and the untouched roster reads much quieter with 6+ enemies. *(→ superseded in round 3: numbers are hover-only now. The roster keeps just the bars; exact numbers live solely on the hover inspect panel, and the panel is narrower for it.)*
