...
public class CrafticsMod implements ModInitializer {
    ...
    private void registerRespawnHooks() {
        ...
        // Every world this mod owns - the overworld lobby, island hubs, arena and raid
        // dimensions - is generated by VoidChunkGenerator, so "a position" and "a position
        // with ground under it" are different things here in a way they are not in vanilla.
        //
        // Vanilla's respawn fallback assumes they are the same. Sleep in a bed, break it,
        // then die: the bed spawn is gone, so the player is dropped at the world spawn of
        // the respawn dimension - a coordinate in empty air. They fall out of the world and
        // die again immediately, this time on a plain vanilla death that drops the entire
        // inventory into the void. The same hole swallows any respawn whose stored spawn
        // block was removed or never had a floor.
        //
        // So validate where the respawn actually put them and reroute if it is over nothing.
        // Runs after the recovery-compass restore above, so a restored inventory travels
        // with the player rather than being dropped by the second death.
        ServerPlayerEvents.AFTER_RESPAWN.register((oldPlayer, newPlayer, alive) -> {
            // alive = returned through the End portal, not a death respawn. That path keeps
            // the player's own position and has no spawn-point fallback to go wrong.
            if (alive) return;
     ...
     */
    /**
     * True when something solid sits within a few blocks under this position, i.e. the player
     * lands instead of falling out of the world. Every world here is void-generated, so this
     * is the check that separates "a valid coordinate" from "somewhere a player can stand".
     */
    public static boolean hasGroundBelow(ServerWorld world, double x, double y, double z) {
        net.minecraft.util.math.BlockPos stand = net.minecraft.util.math.BlockPos.ofFloored(x, y, z);
