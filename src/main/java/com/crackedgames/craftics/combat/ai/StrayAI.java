 ...
 * - KITE: actively retreats if player gets within 2 tiles, then shoots
 * - Prefers maximum distance — more cautious than regular skeleton
...
public class StrayAI implements EnemyAI {
    ...
    public EnemyAction decideAction(CombatEntity self, GridArena arena, GridPos playerPos) {
        ...
        // KITE: if too close, retreat then shoot
        if (dist <= 2) {
        ...
        GridPos shotPos = findKitePosition(self, arena, playerPos);
        if (shotPos != null) {
    ...
    private GridPos findKitePosition(CombatEntity self, GridArena arena, GridPos playerPos) {
        ...
        for (int dx = -self.getMoveSpeed(); dx <= self.getMoveSpeed(); dx++) {
            for (int dz = -self.getMoveSpeed(); dz <= self.getMoveSpeed(); dz++) {
                ...
                // Prefer maximum distance within range
                int score = distToPlayer * 5;
